...
impl CallbackProvider for OOB {
    async fn verifier_code(&self, url: &str) -> Result<String> {
        let msg = format!(
            "please visit and accept the license: {}\ninput pin: \n",
            url,
        );
    ...
    T: Store,
{
    ...
    async fn consumer(&self) -> Result<Credentials> {
        let consumer_key = self.store.get(self.namespace(), API_KEY).and_then(|r| {
            r.ok_or(anyhow!(
                "secret {}@{} not found.",
                API_KEY,
                self.namespace()
        ...
        let consumer_secret = self.store.get(self.namespace(), SECRET_KEY).and_then(|r| {
            r.ok_or(anyhow!(
                "secret {}@{} not found.",
                SECRET_KEY,
                self.namespace()
    ...
    async fn request_token(&self, consumer: &Credentials) -> Result<Credentials> {
        ...
        match (request_token_ts, request_token, request_secret) {
            ...
            _ => {
                ...
                let authorization =
                    oauth::Builder::<_, _>::new(consumer.clone().into(), oauth::HmacSha1)
                        .callback("oob")
                        .get(&uri, &());

                let body = send_request(uri, authorization, &self.client).await;
                let creds: oauth_credentials::Credentials<Box<str>> =
                ...
                self.store.put(
                    self.namespace(),
                    REQUEST_TOKEN_KEY,
                    request_token.key.unsecure(),
                )?;
                self.store.put(
                    self.namespace(),
                    REQUEST_TOKEN_SECRET,
                    request_token.secret.unsecure(),
                )?;
                ...
                let today = Utc::today()
                    ...
                    .to_string();
                self.store
                    .put(self.namespace(), REQUEST_TOKEN_CREATED, &today)?;
                Ok(request_token)
            }
        }
    }
    ...
    async fn access_token(&self, callback: impl CallbackProvider) -> Result<Credentials> {
        ...
        match (access_token, access_secret) {
            ...
            _ => {
                ...
                match self
                    .renew_access_token(&consumer, &request_token.unwrap())
                    .await
                {
                    Ok(access_token) => {
    ...
    async fn full_access_token_flow(
        ...
        let access_token = self
            .create_access_token(&consumer, &request_token, &pin)
            .await?;
    ...
    async fn create_access_token(
        ...
        self.store.put(
            self.namespace(),
            ACCESS_TOKEN_KEY,
            access_token.key.unsecure(),
        )?;
        self.store.put(
            self.namespace(),
            ACCESS_TOKEN_SECRET,
            access_token.secret.unsecure(),
        )?;
        Ok(access_token)
    }
        ...
        C: CallbackProvider + Clone,
    {
        ...
        let (bare_uri, full_uri, params): (&str, String, Option<BTreeSet<(String, String)>>) =
            match &method {
                &Method::GET => {
    ...
    B: http_body::Body<Error = S::Error> + Default + From<Vec<u8>> + Debug,
{
    ...
    if resp.status().as_u16() / 100 == 2 {
        let res = hyper::body::to_bytes(resp.into_body())
            .await
            .unwrap()
            .to_vec();
        res
...
mod tests {
    ...
    fn encodes_query_string() {
        crate::tests::init();
        let data = Some(&[
            ("blah", "some things go here"),
            ("other", "and others go here"),
        ]);
    ...
    fn encodes_json_string() {
        ...
        info!(
            "query string: '{:?}'",
            data.map(|v| hyper::body::Body::from(serde_json::to_string(&v).unwrap()))
