...
use crate::{accounts::Account, Session, Store, SecurityType, Product, MarketSession};
use anyhow::Result;
use strum::EnumString;
use http::Method;
use crate::session::CallbackProvider;
use std::sync::Arc;
      ...
      T: Store,
{
  ...
  pub async fn list_orders(&self, account: &Account, params: ListOrdersRequest, callbacks: impl CallbackProvider) -> Result<OrdersResponse> {
    let portfolio: serde_json::Value = self
        .session
        .send(
          Method::GET,
          format!("/v1/accounts/{}/orders", account.account_id_key),
          qs_params(&params)?,
          callbacks,
        )
        .await?;
    Ok(serde_json::from_value(
      portfolio.get("OrdersResponse").unwrap().clone(),
    )?)
  }
